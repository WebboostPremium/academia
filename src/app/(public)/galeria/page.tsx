...
export default function GaleriaPage() {
  ...
  useEffect(() => {
    Promise.all([getAlbums(true), getGalleryImages(undefined, true)])
      .then(([a, i]) => {
        setAlbums(a);
        setImages(i);
      })
      ...
      <main className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm ${activeAlbum === "all" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
          >
            Todas
          </button>
          {albums.map((a) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-sm ${activeAlbum === a.id ? "bg-primary text-primary-foreground" : "bg-muted"}`}
            >
              {a.title}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <p className="mt-10 text-muted-foreground">Aún no hay imágenes en la galería.</p>
        ) : (
          <div className="mt-8 grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {filtered.map((img) => (
              <button
                ...
                className="group overflow-hidden rounded-xl ring-1 ring-border"
              >
